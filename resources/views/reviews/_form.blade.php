...
<section class="p-review-form__form-content">
    ...
    <form action="{{ route('post.store') }}" method="POST" class="p-review-form__form">
        ...
        <div class="p-review-form__item-group">
            <label for="comment" class="p-review-form__label">{{ __('投稿内容：') }}</label>
            <textarea name="comment" id="comment" cols="100" rows="10" required class="p-review-form__textarea">{{ old('comment') }}</textarea>
        </div>

        {{-- 評価 --}}
        <div class="p-review-form__evaluation">
            <label for="evaluation" class="p-review-form__evaluation-label">{{ __('評価：') }}</label>
